...
public static class DdsLoader
{
    ...
    /// <returns></returns>
    [DllImport("DirectXTexWrapper", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
    private static extern int ExtractDdsTextureData(string filePath, int rawBufferSize, int previewBufferSize,
        byte[] rawImageData, byte[] previewImageData);
    ...
    /// <returns>0 on success, else HRESULT code</returns>
    public static DdsTexture? LoadDdsTexture(string filePath, ILogger? logger = null)
    {
        // Check if the provided file path even exists
        if (!File.Exists(filePath))
        {
            logger?.LogError("The specified file was not found: {FilePath}", filePath);
            return null;
        }
        ...
        var errorCodeInfo =
            ExtractDdsTextureInfo(filePath, out var width, out var height, formatName, out var bitsPerPixel,
                out var bitsPerColor);
        if (errorCodeInfo != 0)
        {
            logger?.LogError("Extracting DDS file data failed with HRESULT {ErrorCodeInfo}", errorCodeInfo);
            return null;
        }
        ...
        logger?.LogInformation(
            ...
            width, height, formatName, bitsPerPixel, bitsPerColor);
        ...
        var errorCodeData = ExtractDdsTextureData(filePath, rawImageData.Length, previewImageData.Length, rawImageData,
            previewImageData);
        if (errorCodeData != 0)
        {
            logger?.LogError("Extracting DDS data failed with HRESULT {ErrorCodeData}", errorCodeData);
            return null;
        }
